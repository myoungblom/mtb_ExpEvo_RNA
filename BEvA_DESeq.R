...
# BF PCA with arrow
B.vst <- vst(B.DESeq, blind=FALSE)
PCAdata <- DESeq2::plotPCA(B.vst, intgroup=c("Clade","Genotype"),returnData=T)
...
rv <- rowVars(assay(B.vst))
select <- order(rv, decreasing = TRUE)[seq_len(min(500,length(rv)))]
pca <- prcomp(t(assay(B.vst)[select, ]))
...
L4.9.t <- DESeq(L4.9.t)
L4.9.vst <- vst(L4.9.t, blind=FALSE)
DESeq2::plotPCA(L4.9.vst, intgroup=c("Strain","Genotype"))
...
L4.4.t <- DESeq(L4.4.t)
L4.4.vst <- vst(L4.4.t, blind=FALSE)
DESeq2::plotPCA(L4.4.vst, intgroup=c("Strain","Genotype"))
...
# lpdA operon expression (Figure S11)
